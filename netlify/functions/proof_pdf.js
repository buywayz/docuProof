...
exports.handler = async (event) => {
  ...
  try {
    const doc = new PDFDocument({
      size: "LETTER",
      margins: { top: inch(0.5), bottom: inch(0.5), left: inch(0.6), right: inch(0.6) },
      info: {
    ...
    // Dots
    doc.circle(centerX - 20, y, 3).fill(green);
    doc.circle(centerX, y, 3).fill(green);
    doc.circle(centerX + 20, y, 3).fill(green);
    y += inch(0.3);
    ...
    const boxL = marginL + inch(0.15);
    const boxW = contentW - inch(0.3);
    const boxH = inch(1.45);
    const boxTop = y;

    doc.lineWidth(1).strokeColor(borderGray).rect(boxL, boxTop, boxW, boxH).stroke();
    doc.rect(boxL, boxTop, boxW, inch(0.32)).fill("#f8f9fa");
    doc.font("Helvetica-Bold").fontSize(9).fillColor(darkGreen);
    doc.text("DOCUMENT DETAILS", boxL + inch(0.2), boxTop + inch(0.1));

    // Details
    const lblX = boxL + inch(0.2);
    const valX = boxL + inch(1.25);
    let rowY = boxTop + inch(0.45);
    const rowH = inch(0.24);
    ...
    for (const [lbl, val] of rows) {
      ...
      doc.font("Helvetica-Bold").fontSize(9).fillColor(black).text(val, valX, rowY, { width: boxW - inch(2.6) });
      rowY += rowH;
    }

    // QR Code
    const qrS = inch(0.95);
    const qrX = boxL + boxW - qrS - inch(0.15);
    const qrY = boxTop + inch(0.4);
    const qrPng = await QRCode.toBuffer(verifyUrl, { width: 190, margin: 0, color: { dark: "#1a1a1a", light: "#ffffff" }, errorCorrectionLevel: "M" });
